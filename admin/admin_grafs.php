...
$quantityAdmins = query("SELECT COUNT(id) FROM users_img WHERE role = 'admin_img'")->fetch_row()[0];
// отсылаем запрос на получения редакторов
$quantityRedactors = query("SELECT COUNT(id) FROM users_img WHERE role = 'redaktor_img'")->fetch_row()[0];
...
function get_tabel_size () {
    $first = mysqli_fetch_array(query('SELECT table_name AS `Table`, round(((data_length + index_length) / 1024 / 1024), 3) `size` FROM information_schema.TABLES WHERE table_schema = "DB_NAME" AND table_name = "markers_img"'))['size'];
    $second = mysqli_fetch_array(query('SELECT table_name AS `Table`, round(((data_length + index_length) / 1024 / 1024), 3) `size` FROM information_schema.TABLES WHERE table_schema = "DB_NAME" AND table_name = "users_img"'))['size'];
    $first = $first + 0;
    $second = $second + 0; 
    return $first + $second;
}
...
<head>
    ...
    <title>Admin obrazkova mapa | info</title>
    <!-- Bootstrap CSS -->
                    ...
                    <div class="col-lg-4">
                        <div class="card mb-3 shadow">
                            <div class="card-header">
                                <i class="fas fa-chart-pie"></i>Poměr uživatelů
                            </div>
                            <div class="card-body">
